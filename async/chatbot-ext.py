""" The following is a cog that you can use in the async liabary of discord.py with ext.commands
        ...
        if not message.author.bot and self.bot.user in message.mentions: # Checking if the bot is being mentioned by another user and not by itself
            try:
                if message.content.startswith(("$", "!", "?", "-", "*", "`", "~", "+", "/", ";", "=", "&", ">")): # a bunch of generic checks to see if the bot is not supposed to reply (for example if you're using another bot and you need to mention our bot, you don't want the bot to reply)
                    ...
                    await self.bot.send_typing(message.channel) # Make it look like the bot is actually typing the message. This adds a nice touch to let the user know the bot has read the message and it processing it
                    ...
                    await self.bot.send_message(message.channel, f"{message.author.mention} {response['result']['fulfillment']['speech']}") # Send the message and tag the message author

            except KeyError: # If the bot gets input dialogflow can't handle, it will raise a KeyError
                await self.bot.send_message(message.channel, "```Error: 'KeyError', insufficiant input```")
